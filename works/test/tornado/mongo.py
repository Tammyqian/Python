...
class MongoIns(object):
    ...
    def m_createIndex(self, table, key, expire_time, **kwargs):
        dbname = None
        if 'dbname' in kwargs:
            dbname = kwargs.pop('dbname')

        host = kwargs.pop('host', None)
        MongoIns().get_conn(host=host, **kwargs)[dbname or DB_NAME][table].create_index([(key, 1)], expireAfterSeconds=expire_time)
